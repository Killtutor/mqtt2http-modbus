...
<jsp:attribute name="subtitle">
  <fmt:message key="header.pointLinks"/>
</jsp:attribute>
<jsp:body>
  <script type="text/javascript">
    ...
    function showPointLink(plId) {
        if (editingPointLink)
            stopImageFader(byId("pl"+ editingPointLink.id +"Img"));
        PointLinksDwr.getPointLink(plId, function(pl) {
            ...
            $set("updateType",pl.updateType);

            startImageFader(byId("pl"+ plId +"Img"));
    ...
    function savePointLink() {
        ...
        PointLinksDwr.savePointLink(editingPointLink.id, $get("xid"), $get("sourcePointId"), $get("targetPointId"),
                $get("script"), $get("event"), $get("disabled"), $get("updateType"), function(response) {
            if (response.hasMessages)
                ...
                if (editingPointLink.id == ${NEW_ID}) {
                    ...
                    startImageFader(byId("pl"+ editingPointLink.id +"Img"));
                    setUserMessage("<fmt:message key="pointLinks.pointLinkAdded"/>");
                    show(byId("deletePointLinkImg"));
                }
                else
    ...
    function updatePointLink(pl) {
        ...
        setPointLinkImg(pl.disabled, byId("pl"+ pl.id +"Img"));
    }
    ...
    function deletePointLink() {
        PointLinksDwr.deletePointLink(editingPointLink.id, function() {
            stopImageFader(byId("pl"+ editingPointLink.id +"Img"));
            byId("pointLinksTable").removeChild(byId("pl"+ editingPointLink.id));
      ...
      <td valign="top" style="display:none;" id="pointLinkDetails">
        <div class="borderDiv">
          ...
          <table>
            ...
            <tr>
              <td class="formLabelRequired"><fmt:message key="pointLinks.updateType"/></td>
              <td class="formField">
                <select id="updateType">
                  <option value="<c:out value="<%= PointLinkVO.UPDATE_TYPE_LOCAL %>"/>"><fmt:message key="pointLinks.updateType.local"/></option>
                  <option value="<c:out value="<%= PointLinkVO.UPDATE_TYPE_DATAPOINT %>"/>"><fmt:message key="pointLinks.updateType.dataPoint"/></option>
                  <option value="<c:out value="<%= PointLinkVO.UPDATE_TYPE_DATASOURCE %>"/>"><fmt:message key="pointLinks.updateType.dataSource"/></option>
                </select>
              </td>
            </tr>
