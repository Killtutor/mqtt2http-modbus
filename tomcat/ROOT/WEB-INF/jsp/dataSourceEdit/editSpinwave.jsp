...
<script type="text/javascript">
  ...
  function refreshSensorAddressList() {
      ...
      var addrSelect = byId("sensorAddress");
  ...
  function swListenCancelCB() {
      swListenButtons(false);
      byId("swListenMessage").innerHTML = "<fmt:message key="common.cancelled"/>";
  }

  function swAddSensorsToList() {
      var sensors = byId("swListenSensors").options;
      for (var i=0; i<sensors.length; i++)
          addAddress(sensors[i].value);
  }

  function writePointListImpl(points) {
      var msgVersion = byId("messageVersion");
  ...
  function savePointImpl(locator) {
      ...
      if ($get("messageVersion") == <%= SpinwaveReceiver.VERSION_1 %>)
          DataSourceEditDwr.saveSpinwaveV1PointLocator(currentPoint.id, $get("xid"), $get("name"), $get("deviceName"), locator,
                  savePointCB);
      else
          DataSourceEditDwr.saveSpinwaveV2PointLocator(currentPoint.id, $get("xid"), $get("name"), $get("deviceName"), locator,
                  savePointCB);
  }
