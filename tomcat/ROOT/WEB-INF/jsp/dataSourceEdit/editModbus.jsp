...
<script type="text/javascript">
    ...
    function savePointImpl(locator) {
        ...
        DataSourceEditDwr.saveModbusPointLocator(currentPoint.id, $get("xid"), $get("name"), $get("deviceName"), locator, savePointCB);
    }
    ...
    function maybeSetDisabled(nodeName, val) {
        var node = byId(nodeName);
        if (node)
            setDisabled(node, val);
    }

    function copyChange() {
        var dup = byId("copy").checked;
        setDisabled(byId("copyCount"), !dup);
        setDisabled(byId("copyName"), !dup);
        setDisabled(byId("copyOffset"), !dup);
        setDisabled(byId("copyBit"), !dup || !copyBit);
        setDisabled(byId("copyButton"), !dup);

        if(!dup)
            $set("copyCount", "1");
    }

    function copyInit(locator){

        if (currentPoint.id == -1) // Common.NEW_ID
          setDisabled(byId("copy"), true);
        else
          setDisabled(byId("copy"), false);

        copyBit=false;
        if (locator.modbusDataType=="<c:out value='<%= DataType.BINARY %>'/>" && (locator.range=="<c:out value='<%= RegisterRange.HOLDING_REGISTER %>'/>"
              || locator.range=="<c:out value='<%= RegisterRange.INPUT_REGISTER %>'/>")){

            copyBit=true;
            if(locator.bit<15){
                $set("copyOffset", locator.offset);
                $set("copyBit", locator.bit + 1);
            }else{
                $set("copyOffset", locator.offset + 1);
                $set("copyBit", 0);
            }
        }else if(locator.modbusDataType=="<c:out value='<%= DataType.BINARY %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.TWO_BYTE_INT_UNSIGNED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.TWO_BYTE_INT_SIGNED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.TWO_BYTE_BCD %>'/>"){

            $set("copyOffset", locator.offset + 1);
            $set("copyBit", 0);
        }else if(locator.modbusDataType=="<c:out value='<%= DataType.FOUR_BYTE_INT_UNSIGNED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.FOUR_BYTE_INT_SIGNED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.FOUR_BYTE_INT_UNSIGNED_SWAPPED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.FOUR_BYTE_INT_SIGNED_SWAPPED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.FOUR_BYTE_FLOAT %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.FOUR_BYTE_FLOAT_SWAPPED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.FOUR_BYTE_BCD %>'/>"){

            $set("copyOffset", locator.offset + 2);
            $set("copyBit", 0);
        }else if(locator.modbusDataType=="<c:out value='<%= DataType.EIGHT_BYTE_INT_UNSIGNED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.EIGHT_BYTE_INT_SIGNED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.EIGHT_BYTE_INT_UNSIGNED_SWAPPED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.EIGHT_BYTE_INT_SIGNED_SWAPPED %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.EIGHT_BYTE_FLOAT %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.EIGHT_BYTE_FLOAT_SWAPPED %>'/>"){

            $set("copyOffset", locator.offset + 4);
            $set("copyBit", 0);
        }else if (locator.modbusDataType=="<c:out value='<%= DataType.CHAR %>'/>"
              || locator.modbusDataType=="<c:out value='<%= DataType.VARCHAR %>'/>"){

            $set("copyOffset", locator.offset + locator.registerCount);
            $set("copyBit", 0);
        }
    ...
    function copyPoint() {
        DataSourceEditDwr.copyModbusPoint(currentPoint.id, $get("copyName"), $get("copyOffset"), $get("copyBit"),
                $get("copyCount"), function(response){

          hide("copyMessage");
          hideContextualMessages(byId("copyPointDiv"));
          if (response.hasMessages)
              showDwrMessages(response.messages);
          else {
              writePointList(response.data.points);
              showMessage("copyMessage", "<fmt:message key="dsEdit.pointSaved"/>");
          }
...
<tag:pointList pointHelpId="modbusPP">
  ...
  <tbody id="nonSlaveMonitor">
    ...
    <tr>
      <td class="formLabelRequired"><fmt:message key="dsEdit.modbus.offset"/></td>
      <td class="formField"><input type="text" id="offset" onBlur="changeDataType('')"/></td>
    </tr>

    <tr>
      <td class="formLabelRequired"><fmt:message key="dsEdit.modbus.bit"/></td>
      <td class="formField"><input id="bit" type="text" onBlur="changeDataType('')"/></td>
    </tr>

    <tr>
      <td class="formLabelRequired"><fmt:message key="dsEdit.modbus.registerCount"/></td>
      <td class="formField"><input id="registerCount" type="text" onBlur="changeDataType('')"/></td>
    </tr>
      ...
      <td colspan="2">
        <div id="copyPointDiv">
          <table style="width:100%">
            ...
            <tr><td colspan="2" id="copyMessage" class="ctxmsg formError"></td></tr>

            <tr>
              <td class="formLabelRequired"><fmt:message key="dsEdit.modbus.copy"/></td>
              <td class="formField"><input id="copy" type="checkbox" onclick="copyChange()"/></td>
            </tr>

            <tr>
              <td class="formLabel"><fmt:message key="dsEdit.name"/></td>
              <td class="formField"><input type="text" id="copyName"/></td>
            </tr>

            <tr>
              <td class="formLabelRequired"><fmt:message key="dsEdit.modbus.offset"/></td>
              <td class="formField"><input type="text" id="copyOffset"/></td>
            </tr>

            <tr>
              <td class="formLabelRequired"><fmt:message key="dsEdit.modbus.bit"/></td>
              <td class="formField"><input type="text" id="copyBit"/></td>
            </tr>

            <tr>
              <td class="formLabelRequired"><fmt:message key="dsEdit.modbus.pointCount"/></td>
              <td class="formField"><input type="text" id="copyCount"/></td>
            </tr>

            <tr>
              <td colspan="2" align="center">
                <input id="copyButton" type="button" value="<fmt:message key="common.copy"/>" onclick="copyPoint();"/>
              </td>
            </tr>

            <tr>
              <td colspan="2"><fmt:message key="dsEdit.modbus.note"/></td>
            </tr>
          </table>
        </div>
      </td>
