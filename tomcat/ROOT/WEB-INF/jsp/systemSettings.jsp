...
<jsp:attribute name="subtitle">
  <fmt:message key="header.systemSettings"/>
</jsp:attribute>
<jsp:body>
  <script type="text/javascript">
    var systemEventAlarmLevels = new Array();
    var auditEventAlarmLevels = new Array();
    var smtpPasswordBuffer;
    var smtp1PasswordBuffer;
    var proxyPasswordBuffer;
    var smtpPasswordChange = false;
    var smtp1PasswordChange = false;
    var proxyPasswordChange = false;
    ...
    function init() {
        SystemSettingsDwr.getSettings(function(settings) {
            ...
            $set("emailSmtpPasswordEmpty", settings.emailSmtpPasswordEmpty);
            if(!settings.emailSmtpPasswordEmpty)
                $set("<c:out value="<%= SystemSettingsDao.EMAIL_SMTP_PASSWORD %>"/>", "****************");

            smtpAuthChange();

            $set("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_HOST %>"/>", settings.<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_HOST %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PORT %>"/>", settings.<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PORT %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EMAIL1_FROM_ADDRESS %>"/>", settings.<c:out value="<%= SystemSettingsDao.EMAIL1_FROM_ADDRESS %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EMAIL1_FROM_NAME %>"/>", settings.<c:out value="<%= SystemSettingsDao.EMAIL1_FROM_NAME %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EMAIL1_AUTHORIZATION %>"/>", settings.<c:out value="<%= SystemSettingsDao.EMAIL1_AUTHORIZATION %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_USERNAME %>"/>", settings.<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_USERNAME %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EMAIL1_TLS %>"/>", settings.<c:out value="<%= SystemSettingsDao.EMAIL1_TLS %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EMAIL1_CONTENT_TYPE %>"/>", settings.<c:out value="<%= SystemSettingsDao.EMAIL1_CONTENT_TYPE %>"/>);
            $set("email1SmtpPasswordEmpty", settings.email1SmtpPasswordEmpty);
            if(!settings.email1SmtpPasswordEmpty)
                $set("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PASSWORD %>"/>", "****************");
            ...
            $set("httpClientProxyPasswordEmpty", settings.httpClientProxyPasswordEmpty);
            if(!settings.httpClientProxyPasswordEmpty)
                $set("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_PASSWORD %>"/>", "****************");
            ...
            $set("<c:out value="<%= SystemSettingsDao.TEXT_MESSAGE_PURGE_PERIOD_TYPE %>"/>", settings.<c:out value="<%= SystemSettingsDao.TEXT_MESSAGE_PURGE_PERIOD_TYPE %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.TEXT_MESSAGE_PURGE_PERIODS %>"/>", settings.<c:out value="<%= SystemSettingsDao.TEXT_MESSAGE_PURGE_PERIODS %>"/>);
            ...
            $set("<c:out value="<%= SystemSettingsDao.MAX_WRONG_PASSWORDS %>"/>", settings.<c:out value="<%= SystemSettingsDao.MAX_WRONG_PASSWORDS %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.USER_UNLOCK_MANUAL %>"/>", settings.<c:out value="<%= SystemSettingsDao.USER_UNLOCK_MANUAL %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.USER_UNLOCK_PERIODS %>"/>", settings.<c:out value="<%= SystemSettingsDao.USER_UNLOCK_PERIODS %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.USER_UNLOCK_PERIOD_TYPE %>"/>", settings.<c:out value="<%= SystemSettingsDao.USER_UNLOCK_PERIOD_TYPE %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.CAPTCHA_ENABLED %>"/>", settings.<c:out value="<%= SystemSettingsDao.CAPTCHA_ENABLED %>"/>);
            ...
            var sel = byId("<c:out value="<%= SystemSettingsDao.LANGUAGE %>"/>");
    ...
    function saveEmailSettings() {
        SystemSettingsDwr.saveEmailSettings(
            ...
            smtpPasswordChange || byId("emailSmtpPasswordEmpty").checked,
    ...
    function saveEmail1Settings() {
        SystemSettingsDwr.saveEmail1Settings(
            $get("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_HOST %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PORT %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EMAIL1_FROM_ADDRESS %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EMAIL1_FROM_NAME %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EMAIL1_AUTHORIZATION %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_USERNAME %>"/>"),
            smtp1PasswordChange || byId("email1SmtpPasswordEmpty").checked,
            $get("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PASSWORD %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EMAIL1_TLS %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EMAIL1_CONTENT_TYPE %>"/>"),
            function() {
                stopImageFader("saveEmail1SettingsImg");
                setUserMessage("email1Message", "<fmt:message key="systemSettings.emailSettingsSaved"/>");
            });
        setUserMessage("email1Message");
        startImageFader("saveEmail1SettingsImg");
    }
    ...
    function sendTestEmail1() {
        SystemSettingsDwr.sendTestEmail1(
                function(result) {
                    stopImageFader("sendTestEmail1Img");
                    if (result.exception)
                        setUserMessage("email1Message", result.exception);
                    else
                        setUserMessage("email1Message", result.message);
                });
        setUserMessage("email1Message");
        startImageFader("sendTestEmail1Img");
    }
    ...
    function smtpAuthChange() {
        var auth = byId("<c:out value="<%= SystemSettingsDao.EMAIL_AUTHORIZATION %>"/>").checked;
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.EMAIL_SMTP_USERNAME %>"/>"), !auth);
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.EMAIL_SMTP_PASSWORD %>"/>"), !auth || byId("emailSmtpPasswordEmpty").checked);
        setDisabled(byId("emailSmtpPasswordEmpty"), !auth);
    }

    function smtp1AuthChange() {
        var auth = byId("<c:out value="<%= SystemSettingsDao.EMAIL1_AUTHORIZATION %>"/>").checked;
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_USERNAME %>"/>"), !auth);
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PASSWORD %>"/>"), !auth || byId("email1SmtpPasswordEmpty").checked);
        setDisabled(byId("email1SmtpPasswordEmpty"), !auth);
    }
    ...
    function saveHttpSettings() {
        SystemSettingsDwr.saveHttpSettings(
                ...
                proxyPasswordChange || byId("httpClientProxyPasswordEmpty").checked,
    ...
    function httpUseProxyChange() {
        var proxy = byId("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_USE_PROXY %>"/>").checked;
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_SERVER %>"/>"), !proxy);
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_PORT %>"/>"), !proxy);
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_USERNAME %>"/>"), !proxy);
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_PASSWORD %>"/>"), !proxy || byId("httpClientProxyPasswordEmpty").checked);
        setDisabled(byId("httpClientProxyPasswordEmpty"), !proxy);
    }

    function saveSecuritySettings() {
        setUserMessage("securityMessage");
        startImageFader("saveSecuritySettingsImg");
        SystemSettingsDwr.saveSecuritySettings(
                $get("<c:out value="<%= SystemSettingsDao.MAX_WRONG_PASSWORDS %>"/>"),
                $get("<c:out value="<%= SystemSettingsDao.USER_UNLOCK_MANUAL %>"/>"),
                $get("<c:out value="<%= SystemSettingsDao.USER_UNLOCK_PERIODS %>"/>"),
                $get("<c:out value="<%= SystemSettingsDao.USER_UNLOCK_PERIOD_TYPE %>"/>"),
                $get("<c:out value="<%= SystemSettingsDao.CAPTCHA_ENABLED %>"/>"),
                function() {
                    stopImageFader("saveSecuritySettingsImg");
                    setUserMessage("securityMessage", "<fmt:message key="systemSettings.securitySaved"/>");
                }
        );
    }
    ...
    function saveMiscSettings() {
        ...
        SystemSettingsDwr.saveMiscSettings(
                ...
                $get("<c:out value="<%= SystemSettingsDao.TEXT_MESSAGE_PURGE_PERIOD_TYPE %>"/>"),
                $get("<c:out value="<%= SystemSettingsDao.TEXT_MESSAGE_PURGE_PERIODS %>"/>"),
                function() {
    ...
    function acknowledgeAllEvents() {
        if (confirm("<fmt:message key="systemSettings.acknowledgeEventsConfirm"/>")) {
            setUserMessage("miscMessage", "<fmt:message key="systemSettings.acknowledgeEventsInProgress"/>");
            MiscDwr.acknowledgeAllUsersPendingEvents(function(msg) {
                setUserMessage("miscMessage", msg);
            });
        }
    }

    function smtpPasswordOnChange(){
        console.log("changed");
        smtpPasswordChange = true;
        smtpPasswordBuffer = $get("<c:out value="<%= SystemSettingsDao.EMAIL_SMTP_PASSWORD %>"/>")
    }

    function smtpPasswordOnFocus(){
        smtpPasswordBuffer = $get("<c:out value="<%= SystemSettingsDao.EMAIL_SMTP_PASSWORD %>"/>")
        $set("<c:out value="<%= SystemSettingsDao.EMAIL_SMTP_PASSWORD %>"/>", "");
    }

    function smtpPasswordOnBlur(){
        $set("<c:out value="<%= SystemSettingsDao.EMAIL_SMTP_PASSWORD %>"/>", smtpPasswordBuffer);
    }

    function smtpPasswordEmptyChange() {
        var empty = byId("emailSmtpPasswordEmpty").checked;
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.EMAIL_SMTP_PASSWORD %>"/>"), empty);
        if(empty)
            smtpPasswordOnFocus();
        else
            smtpPasswordOnBlur();
    ...
    function smtp1PasswordOnChange(){
        console.log("changed1");
        smtp1PasswordChange = true;
        smtp1PasswordBuffer = $get("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PASSWORD %>"/>")
    }

    function smtp1PasswordOnFocus(){
        smtp1PasswordBuffer = $get("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PASSWORD %>"/>")
        $set("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PASSWORD %>"/>", "");
    }

    function smtp1PasswordOnBlur(){
        $set("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PASSWORD %>"/>", smtp1PasswordBuffer);
    }

    function smtp1PasswordEmptyChange() {
        var empty = byId("email1SmtpPasswordEmpty").checked;
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PASSWORD %>"/>"), empty);
        if(empty)
            smtp1PasswordOnFocus();
        else
            smtp1PasswordOnBlur();
    }

    function proxyPasswordOnChange(){
        console.log("changed1");
        proxyPasswordChange = true;
        proxyPasswordBuffer = $get("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_PASSWORD %>"/>")
    }

    function proxyPasswordOnFocus(){
        proxyPasswordBuffer = $get("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_PASSWORD %>"/>")
        $set("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_PASSWORD %>"/>", "");
    }

    function proxyPasswordOnBlur(){
        $set("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_PASSWORD %>"/>", proxyPasswordBuffer);
    }

    function proxyPasswordEmptyChange() {
        var empty = byId("httpClientProxyPasswordEmpty").checked;
        setDisabled(byId("<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_PASSWORD %>"/>"), empty);
        if(empty)
            proxyPasswordOnFocus();
        else
            proxyPasswordOnBlur();
    }
  </script>
  ...
  <div class="borderDiv marB marR" style="clear:left;float:left;">
    ...
    <table>
      ...
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.emptyPassword"/></td>
        <td class="formField">
          <input id="emailSmtpPasswordEmpty" type="checkbox" onclick="smtpPasswordEmptyChange()"/>
        </td>
      </tr>
      <tr>
        <td class="formLabel"><fmt:message key="systemSettings.smtpPassword"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EMAIL_SMTP_PASSWORD %>"/>" type="password"
                onChange="smtpPasswordOnChange();" onfocus="smtpPasswordOnFocus();" onblur="smtpPasswordOnBlur();"/></td>
      </tr>
      <tr>
  ...
  <div class="borderDiv marB marR" style="float:left;">
    <table width="100%">
      <tr>
        <td>
          <span class="smallTitle"><fmt:message key="systemSettings.email1Settings"/></span>
          <tag:help id="emailSettings"/>
        </td>
        <td align="right">
          <tag:img id="saveEmail1SettingsImg" png="save" onclick="saveEmail1Settings();" title="common.save"/>
          <tag:img id="sendTestEmail1Img" png="email_go" onclick="sendTestEmail1();" title="common.sendTestEmail"/>
        </td>
      </tr>
    </table>
    <table>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.smtpHost"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_HOST %>"/>" type="text"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.smtpPort"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PORT %>"/>" type="text"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.fromAddress"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EMAIL1_FROM_ADDRESS %>"/>" type="text"/></td>
      </tr>
      <tr>
        <td class="formLabel"><fmt:message key="systemSettings.fromName"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EMAIL1_FROM_NAME %>"/>" type="text"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.auth"/></td>
        <td class="formField">
          <input id="<c:out value="<%= SystemSettingsDao.EMAIL1_AUTHORIZATION %>"/>" type="checkbox" onclick="smtp1AuthChange()"/>
        </td>
      </tr>
      <tr>
        <td class="formLabel"><fmt:message key="systemSettings.smtpUsername"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_USERNAME %>"/>" type="text"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.emptyPassword"/></td>
        <td class="formField">
          <input id="email1SmtpPasswordEmpty" type="checkbox" onclick="smtp1PasswordEmptyChange()"/>
        </td>
      </tr>
      <tr>
        <td class="formLabel"><fmt:message key="systemSettings.smtpPassword"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EMAIL1_SMTP_PASSWORD %>"/>" type="password"
                onChange="smtp1PasswordOnChange();" onfocus="smtp1PasswordOnFocus();" onblur="smtp1PasswordOnBlur();"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.tls"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EMAIL1_TLS %>"/>" type="checkbox"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.contentType"/></td>
        <td class="formField">
          <select id="<c:out value="<%= SystemSettingsDao.EMAIL1_CONTENT_TYPE %>"/>">
            <option value="<c:out value="<%= MangoEmailContent.CONTENT_TYPE_BOTH %>"/>"><fmt:message key="systemSettings.contentType.both"/></option>
            <option value="<c:out value="<%= MangoEmailContent.CONTENT_TYPE_HTML %>"/>"><fmt:message key="systemSettings.contentType.html"/></option>
            <option value="<c:out value="<%= MangoEmailContent.CONTENT_TYPE_TEXT %>"/>"><fmt:message key="systemSettings.contentType.text"/></option>
          </select>
      ...
      <tr>
        <td colspan="2" id="email1Message" class="formError"></td>
      </tr>
    </table>
  </div>
    ...
    <table id="miscSettingsTab">
      ...
      <tr>
        <td colspan="2" align="center">
          <input type="button" value="<fmt:message key="systemSettings.acknowledgeEvents"/>" onclick="acknowledgeAllEvents()"/>
        </td>
      </tr>
      <tr>
      ...
      <tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.purgeTextMessages"/></td>
        <td class="formField">
          <input id="<c:out value="<%= SystemSettingsDao.TEXT_MESSAGE_PURGE_PERIODS %>"/>" type="text" class="formShort"/>
          <select id="<c:out value="<%= SystemSettingsDao.TEXT_MESSAGE_PURGE_PERIOD_TYPE %>"/>">
            <tag:timePeriodOptions d="true" w="true" mon="true" y="true"/>
          </select>
        </td>
      </tr>
  ...
  <div class="borderDiv marB marR" style="float:left;">
    ...
    <table>
      ...
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.emptyPassword"/></td>
        <td class="formField">
          <input id="httpClientProxyPasswordEmpty" type="checkbox" onclick="proxyPasswordEmptyChange()"/>
        </td>
      </tr>
      <tr>
        <td class="formLabel"><fmt:message key="systemSettings.proxyPassword"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.HTTP_CLIENT_PROXY_PASSWORD %>"/>" type="password"
                onChange="proxyPasswordOnChange();" onfocus="proxyPasswordOnFocus();" onblur="proxyPasswordOnBlur();"/></td>
      </tr>
      <tr>
  ...
  <div class="borderDiv marB marR" style="float:left;clear:left;">
    <table width="100%">
      <tr>
        <td>
          <span class="smallTitle"><fmt:message key="systemSettings.securitySettings"/></span>
          <tag:help id="securitySettings"/>
        </td>
        <td align="right">
          <tag:img id="saveSecuritySettingsImg" png="save" onclick="saveSecuritySettings();" title="common.save"/>
        </td>
      </tr>
    </table>
    <table>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.maxWrongPasswords"/></td>
        <td class="formField">
          <input id="<c:out value="<%= SystemSettingsDao.MAX_WRONG_PASSWORDS %>"/>" type="text" class="formShort"/>
        </td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.userUnlockTime"/></td>
        <td class="formField">
          <input id="<c:out value="<%= SystemSettingsDao.USER_UNLOCK_PERIODS %>"/>" type="text" class="formShort"/>
          <select id="<c:out value="<%= SystemSettingsDao.USER_UNLOCK_PERIOD_TYPE %>"/>">
            <tag:timePeriodOptions min="true" h="true" d="true"/>
          </select>
        </td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.userUnlockManual"/></td>
        <td class="formField"><input type="checkbox" id="<c:out value="<%= SystemSettingsDao.USER_UNLOCK_MANUAL %>"/>"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.captchaEnabled"/></td>
        <td class="formField"><input type="checkbox" id="<c:out value="<%= SystemSettingsDao.CAPTCHA_ENABLED %>"/>"/></td>
      </tr>
      <tr>
        <td colspan="2" id="securityMessage" class="formError"></td>
      </tr>
    </table>
  </div>
