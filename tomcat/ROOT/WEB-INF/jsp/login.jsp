  ...
  <script type="text/javascript">
    ...
    function setFocus() {
        ...
        if (checkCombo(BrowserDetect.browser, BrowserDetect.version, BrowserDetect.OS)) {
            byId("browserImg").src = "images/accept.png";
            show("okMsg");
            compatible = true;
        }
        else {
            byId("browserImg").src = "images/thumb_down.png";
            show("warnMsg");
        }
    }
        ...
        <form action="login.htm" method="post" onclick="nag()">
          ...
          <table>
            ...
            <c:if test="${captchaEnabled}">
              <tr>
                <td colspan="2" style="text-align:right;"><img src="captcha" id="captchaImg"/></td>
              </tr>

              <spring:bind path="login.captcha">
                <tr>
                  <td class="formLabelRequired"><fmt:message key="login.captcha"/></td>
                  <td class="formField">
                    <input id="password" type="captcha" name="captcha" value="${status.value}" maxlength="20"/>
                  </td>
                  <td class="formError">${status.errorMessage}</td>
                </tr>
              </spring:bind>
