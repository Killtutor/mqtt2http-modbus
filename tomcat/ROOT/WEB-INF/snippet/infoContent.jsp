...
<c:if test="${!empty sessionUser && !empty point}">
  <c:choose>
    <c:when test="${point.loggingType==3}">
      <tag:img png="icon_comp" title="watchlist.pointDetails" style="display:inline; opacity:0.5; filter=alpha(opacity=50)"
              onclick="window.location='data_point_details.shtm?dpid=${point.id}'"/>
    </c:when>
    <c:otherwise>
      <tag:img png="icon_comp" title="watchlist.pointDetails" style="display:inline" onclick="window.location='data_point_details.shtm?dpid=${point.id}'"/>
    </c:otherwise>
  </c:choose>
</c:if>
