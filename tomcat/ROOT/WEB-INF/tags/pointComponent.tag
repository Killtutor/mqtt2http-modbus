...
<c:choose>
  ...
  <c:when test="${vc.pointComponent}">
    <div id="c${vc.id}" style="${vc.style}"
      ...
      <div id="c${vc.id}StaticContent">${vc.staticContent}</div>
      <c:if test="${vc.displayControls}">
        <div id="c${vc.id}Controls" class="controlsDiv">
          <table cellpadding="0" cellspacing="1">
            ...
            <c:if test="${vc.pointDetectors}">
              <tr onclick="showPointDetectors('${vc.dataPointId}',function(){})"><td>
                <img src="images/bell.png" alt=""/>
                <div id="p${vc.dataPointId}HourGlass"style="visibility:hidden;" class="labelDiv" ><tag:img png="hourglass" title="common.gettingData"/></div>
              </td></tr>
            </c:if>
          </table>
        </div>
