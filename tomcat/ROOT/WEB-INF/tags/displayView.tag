...
<c:if test="${!empty view}">
  <script type="text/javascript">
      ViewDwr.getJavascriptComponents(${view.id}, function(components){
          mango.view.initScripts(components);
      });
  </script>
</c:if>
<div id="hourglass"><tag:img png="hourglass"/></div>
<div id="viewContent" style="display: inline-block; display:none;">
  <c:choose>
    ...
    <c:choose>
        ...
        <div id="c${vc.id}" style="position:absolute;left:${vc.x}px;top:${vc.y}px;"
          ...
          <div id="c${vc.id}Controls" class="controlContent" style="left:5px;top:5px;${bkgd}">
            ...
            <c:forEach items="${vc.childComponents}" var="child">
              <c:if test="${child.pointComponent.visible && child.pointComponent.id != vc.leadComponent.id}">
                <tag:pointComponent vc="${child.pointComponent}"/>
              </c:if>
            </c:forEach>
          </div>
        </div>
      ...
      <c:when test="${vc.compoundComponent}">
        <div id="c${vc.id}" style="position:absolute;left:${vc.x}px;top:${vc.y}px;"
          ...
          <c:forEach items="${vc.childComponents}" var="child">
            <c:if test="${child.pointComponent.visible}"><tag:pointComponent vc="${child.pointComponent}"/></c:if>
          </c:forEach>
        </div>
      </c:when>

      <c:when test="${vc.staticComponent}">
        <tag:staticComponent vc="${vc}"/>
      </c:when>

      <c:when test="${vc.metaComponent}">
        <c:choose>
          <c:when test="${!vc.visible}"></c:when>
          <c:when test="${vc.defName == 'htmlMeta'}">
            <div id="c${vc.id}" style="position:absolute;left:${vc.x}px;top:${vc.y}px;">
              <div id="c${vc.id}Content"><img src="images/icon_comp.png" alt=""/></div>
              <div id="c${vc.id}StaticContent">${vc.htmlContent}</div>
            </div>
          </c:when>
          <c:otherwise>
            <div id="c${vc.id}" style="position:absolute;left:${vc.x}px;top:${vc.y}px;"></div>
          </c:otherwise>
        </c:choose>
      </c:when>
