...
mango.view.setMessages = function(state) {
    var warningNode = byId("c"+ state.id +"Warning");
...
mango.view.setContent = function(state) {
    if (state.content != null) {
        var cont = byId("c"+ state.id +"Content");
        var dynCont = byId("c"+ state.id +"DynContent");
        var statCont = byId("c"+ state.id +"StaticContent");

        if(statCont && (statCont.innerHTML != "")){
            if(dynCont)
                dynCont.innerHTML = state.content;
            else
                byId("c"+ state.id).savedState = state;

            cont.innerHTML = "";
        }else{
            cont.innerHTML = state.content;
        }

        var dyn = byId("dyn"+ state.id);
        if (dyn) {
...
mango.view.initScripts = function (components){
    var i;
    for(i=0;i<components.length;i++)
        mango.view.updateJavascriptComponentContent("c" + components[i].id, components[i].content);
};

mango.view.updateJavascriptComponentContent = function (id, content){
    if(content && content != ""){
        var oldScript = byId(id + "Script");
        if (!oldScript){
            var jscript = document.createElement('script');
            jscript.setAttribute('type', 'text/javascript');
            jscript.text = content;
            jscript.id = id + "Script";
            document.getElementsByTagName('head')[0].appendChild(jscript);
        }else{
            oldScript.text = content;
        }
    }else{
        var oldScript = byId(id + "Script");
            if (oldScript)
    ...
    var comp = byId(id + "Content");
    if (comp)
        $set(id + "Content", '<img src="images/script_code.png" alt=""/>');
};

mango.view.showChange = function(divId, xoffset, yoffset) {
    mango.view.setEditing = true;
    var theDiv = byId(divId);
...
mango.view.watchList.setDataImpl = function(state) {
    // Check that the point exists. Ignore if it doesn't.
    if (state && byId("p"+ state.id)) {
        var node;
        if (state.value != null) {
            node = byId("p"+ state.id +"Value");
        ...
        if (state.change != null) {
            show(byId("p"+ state.id +"ChangeMin"));
            if (!mango.view.setEditing)
                $set("p"+ state.id +"Change", state.change);
        }

        if (state.chart != null) {
            show(byId("p"+ state.id +"ChartMin"));
            $set("p"+ state.id +"Chart", state.chart);
        }

        if (state.logging == true) {
            document.getElementById("p"+ state.id +"ChartCB").disabled=false;
        }else{
            setOpacity(byId("p"+ state.id +"Details"), 0.5);
        }

        if (state.detectors == true) {
            show("p" + state.id + "pointDetectorsEdit");
        }

        if (state.messages != null)
            byId("p"+ state.id +"Messages").innerHTML = state.messages;
        //else
        //    byId("p"+ state.id +"Messages").innerHTML = "";
    }
};
...
mango.view.pointDetails.setData = function(state) {
    if (state.value != null)
        byId("pointValue").innerHTML = state.value;

    if (state.time != null)
        byId("pointValueTime").innerHTML = state.time;

    if (state.change != null) {
        show(byId("pointChangeNode"));
        $set("pointChange", state.change);
    }

    if (state.messages != null)
        byId("pointMessages").innerHTML = state.messages;
};
