...
window.onbeforeunload = function () { mango.unloading = true; };
...
mango.longPoll.pollCB = function(response) {
    ...
    if (typeof(response.highestUnsilencedAlarmLevel) != "undefined") {
        if (response.highestUnsilencedAlarmLevel > 0) {
            setAlarmLevelImg(response.highestUnsilencedAlarmLevel, byId("__header__alarmLevelImg"));
            setAlarmLevelText(response.highestUnsilencedAlarmLevel, byId("__header__alarmLevelText"));
...
function setOpacity (img, opacity) {
    ...
    img.style.filter = "alpha(opacity=" + (opacity *100) + ")"; //For IE;
}

function isShowing (node) {
    return byId(node).style.display != "none";
}
...
function getNodeBounds(node) {
    node = getNodeIfString(node);
    var box = dojo.getContentBox(node);
    return {
        x : node.offsetLeft,
        y : node.offsetTop,
        w : box.w,
...
function createFromTemplate(templateId, id, parentId) {
    var content = byId(templateId).cloneNode(true);
    updateTemplateNode(content, id);
    content.mangoId = id;
    byId(parentId).appendChild(content);
    show(content);
    return content;
}
...
function getAlarmLevelHtmlImg(alarmLevel) {
    if (alarmLevel == 0)
        return "<img src='images/flag_green.png' title='" + mango.i18n['common.alarmLevel.none'] + "'/>";
    else if (alarmLevel == 1)
        return "<img src='images/flag_blue.png' title='" + mango.i18n['common.alarmLevel.info'] + "'/>";
    else if (alarmLevel == 2)
        return "<img src='images/flag_yellow.png' title='" + mango.i18n['common.alarmLevel.urgent'] + "'/>";
    else if (alarmLevel == 3)
        return "<img src='images/flag_orange.png' title='" + mango.i18n['common.alarmLevel.critical'] + "'/>";
    else if (alarmLevel == 4)
        return "<img src='images/flag_red.png' title='" + mango.i18n['common.alarmLevel.lifeSafety'] + "'/>";
}
...
function setTextGatewayStatusImg(enabled, imgNode) {
    if (enabled)
        updateImg(imgNode, "images/phone_go.png", mango.i18n["common.enabledToggle"], true);
    else
        updateImg(imgNode, "images/phone_stop.png", mango.i18n["common.disabledToggle"], true);
}
...
function hideContextualMessages(parent) {
    ...
    var nodes = parent.getElementsByClassName("ctxmsg");
...
function showDwrMessages(/*DwrResponseI18n.messages*/messages, /*tbody*/genericMessageNode) {
    ...
    for (var i=0; i<messages.length; i++) {
        m = messages[i];
        if (m.contextKey) {
            node = byId(m.contextKey +"Ctxmsg");
            if (!node) {
                field = byId(m.contextKey);
                if (field)
...
function setSilenced(eventId, silenced) {
    var imgNode = byId("silenceImg"+ eventId);
    if (silenced)
...
function ackEvent(eventId) {
    ...
    var imgNode = byId("ackImg"+ eventId);
